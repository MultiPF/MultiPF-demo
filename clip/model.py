...
class AttentionConv(nn.Module):
    ...
    def forward(self, x):
        ...
        k1 = F.linear(x, self.k_proj.weight, self.k_proj.bias)
        x1 = F.linear(x, self.v_proj.weight, self.v_proj.bias)
        x1 = F.linear(x1, self.c_proj.weight, self.c_proj.bias)
        # x2 dimension is [50, n 1024]
        x1 = x1.permute(1, 2, 0)  # n,1024,50
        # print('x2x2x2x2x2x2x2x2x2x2x2x2x2x2x2x2x2x2x2x2x2x2')
        # print(x2.size())
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual_pfa = ModifiedResNet_conv_proj(
                layers=vision_layers,
                output_dim=embed_dim,
                heads=vision_heads,
                input_resolution=image_resolution,
...
def build_model(state_dict: dict):
    ...
    visual_keys = [key for key in state_dict.keys() if "visual" in key]
    visual_params = {key: state_dict[key] for key in visual_keys}

    for key in clip_state_dict.keys():
        if "visual_pfa" in key:
            key_v = key.replace("visual_pfa", "visual")
            clip_state_dict[key] = visual_params[key_v]
        else:
            clip_state_dict[key] = state_dict[key]
